...
download_service() {
    curl -s https://raw.githubusercontent.com/mofhusseini/cronos/master/docs/gravity-bridge/systemd/gorc.service.template -o gorc.service.template
}

cleanup_local_serivce_file() {
...
setup_service() {
    sed "s#<GORC_BINARY>#$GORC_BINARY#g; s#<GORC_USER>#$GORC_USER#g; s#<GORC_BINARY_DIR>#$GORC_BINARY_DIR#g; s#<GORC_USER_HOME>#$GORC_USER_HOME#g" $BASEDIR/gorc.service.template > $BASEDIR/gorc.service
